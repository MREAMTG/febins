...
RUN if [ -n "${APT_CMD}" ] && [ "$(uname -m)" = "x86_64" ]; then \
  ...
  export gccMajorVersion="$(echo ${GCC_VERSION} | cut -d. -f1)"; ../configure --enable-languages=c,c++,fortran --prefix=${FE_DIR}/gcc --disable-multilib --disable-multi-arch \
...
COPY --from=gcc_build /opt/FactoryEngine/ /opt/FactoryEngine/
